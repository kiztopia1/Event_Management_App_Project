  ...
  db.serialize(() => {
    ...
    db.run(
      ...
      function (err) {
        ...
        // Select three random employees and assign them to the event
        const selectEmployeesQuery =
          "SELECT id FROM employees ORDER BY RANDOM() LIMIT 3";
        db.all(selectEmployeesQuery, (err, employees) => {
          if (err) {
            console.error("Error selecting employees:", err.message);
            // Rollback the transaction if there's an error
            db.run("ROLLBACK");
            return res.status(500).json({ message: "Internal server error" });
          }
          ...
          if (employees.length < 3) {
            console.error("Not enough employees to assign");
            // Rollback the transaction if there are not enough employees
            db.run("ROLLBACK");
            return res
              .status(500)
              .json({ message: "Not enough employees to assign" });
          }

          // Insert selected employees into the event_employees table
          const insertEventEmployeesQuery =
            "INSERT INTO event_employees (event_id, employee_id) VALUES (?, ?)";
          employees.forEach((employee) => {
            db.run(insertEventEmployeesQuery, [eventId, employee.id], (err) => {
              if (err) {
